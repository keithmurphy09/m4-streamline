...
function renderSchedule() {
    ...
    return `<div><div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6"><h2 class="text-2xl font-bold dark:text-teal-400">Schedule</h2><div class="flex flex-wrap gap-2 sm:gap-4">${viewToggle}<button onclick="exportToCSV('jobs')" class="bg-gray-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-gray-700 text-sm whitespace-nowrap">Export CSV</button><button onclick="openModal('job')" class="bg-black text-white px-3 sm:px-4 py-2 rounded-lg border border-teal-400 text-sm whitespace-nowrap">+ Schedule Job</button></div></div><input type="text" id="job-search-input" placeholder="🔍 Search jobs..." value="${jobSearch}" oninput="jobSearch = this.value; clearTimeout(window.jobSearchTimer); window.jobSearchTimer = setTimeout(() => { currentPage.jobs = 1; renderApp(); setTimeout(() => document.getElementById('job-search-input')?.focus(), 0); }, 300);" class="w-full px-4 py-2 border rounded dark:bg-gray-700 dark:text-white dark:border-gray-600-lg dark:bg-gray-700 dark:text-white dark:border-gray-600 mb-4">${bulkActions}${selectAllCheckbox}<div class="grid gap-4">${jobsList}</div>${pagination}</div>`;
}
